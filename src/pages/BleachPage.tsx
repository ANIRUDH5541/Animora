...
const BleachPage: React.FC = () => {
  const [filters, setFilters] = useState({});
  const products = getProductsByTheme('bleach');
  const themeColors = ['Black', 'White', 'Gray', 'Silver'];
      ...
      <div className="relative h-[50vh] overflow-hidden">
        ...
        <div className="relative z-20 h-full flex flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-xl text-gray-300 max-w-2xl">
            Unleash your Soul Reaper spirit with our exclusive Bleach merchandise featuring iconic characters and symbols.
          </p>
        </div>
      </div>
          ...
          <div className="md:col-span-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {products.map(product => (
                <ProductCard key={product.id} product={product} theme="bleach" />
              ))}
            </div>
          </div>
        </div>
